...
<h3>Pergunta 1: Qual o gênero de filme mais popular?</h3>
Já que não vamos fazer bootstrap, podemos trabalhar com um amostra maior, 100 mil observações. Pelos intervalos de confiança que calculamos sabemos que três gêneros podem ser os mais populares. Vamos focar apenas em *Comédia* e *Drama*. Nossa hipótese nula será: *Existe uma diferença significativa entra as popularidades dos filmes de Comédia e Drama?*
```{r}
...
Agora podemos guardar a diferença observada entre os gêneros de Comédia e Drama.

```{r}

diff_comedy_drama = count_genres[5,3] - count_genres[4,3]

set.seed(123)
diffs = replicate(1000, {

  amostra2 = sample_n(dados, 100000) %>% filter(genre %in% genres)
  
  
  count_genres_2 <- plyr::count(amostra2, 'genre') %>% mutate(prop = freq / (sum(freq)))
  
  
  return(count_genres_2[5,3] - count_genres_2[4,3]) # comedia e drama
})

tibble(diferenca = diffs) %>% 
  ggplot(aes(x = diferenca)) + 
  geom_histogram(bins = 30) + 
  geom_vline(xintercept = diff_comedy_drama, color="orange")

# p-valor!
sum(abs(diffs) >= abs(diff_comedy_drama)) / length(diffs)
